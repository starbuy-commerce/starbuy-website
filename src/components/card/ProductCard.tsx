...
export default function ProductCard({ img, name, price, id }: Props) {
    ...
    return (
        <>
            <div className="z-0 flex-grow cursor-pointer justify-center box-border flex-row w-24 max-w-[10rem] md:max-w-[13rem] md:w-52 md:shadow-md border-yellow-400 rounded-lg border shadow-gray-300 transition ease-in-out hover:border-violet-700 delay-100 hover:-translate-y-1 hover:scale-110 duration-300" onClick={() => window.location.href="/item/" + id}>
                <img src={img} className="p-4 w-24 h-24 md:h-52 md:w-52 rounded-t-md object-cover" />
                <div className="w-24 h-20 md:w-52 md:h-24">
