...
import {Rating, Skeleton, Snackbar, ThemeProvider} from "@mui/material";
                    ...
                    <div className="md:w-3/5 flex flex-col justify-between">
                        <div>
                            {title !== ""
                                ...
                                : <div>
                                    <Skeleton variant="text" sx={{fontSize: '1rem'}} className="mb-4"/>
                                    <Skeleton variant="text" sx={{fontSize: '1rem'}}/>
                                </div>
                            }
                            <div className="font-inter text-4xl font-normal text-gray-800 static mt-8 ml-2 flex">
                                <p className="text-2xl mr-2">R$</p>
                                <p className="font-semibold">{numberWithCommas(preco.toFixed(2))}</p>
                            </div>
                            <div className="flex">
                                <div className="w-1/2">
                                    <div
                                        className="flex text-sm font-inter font-semibold my-auto ml-2 mr-2 text-gray-700">
                                        <p className="mt-1 mr-2">Avaliação: </p>
                                        {rating == -1 ? <p className="w-full"> Nenhuma avaliação</p> :
                                            <Rating className="mt-1" precision={0.5} name="read-only" value={rating / 2}
                                                    readOnly size="small"/>
                                        }
                                    </div>
                                </div>
                        ...
                        <div className="md:float-right flex mt-10">
                            <div onClick={postCart}
                                 className="mr-4 text-sm font-inter font-semibold bg-transparent text-indigo-500 py-2 px-3 border border-indigo-500 rounded hover:cursor-pointer">
                                <div className="flex">
                                    <img src={addToCart} alt=""/>
                                    <span className="ml-2">Adicionar ao carrinho</span>
                                </div>
                            </div>
                            <div onClick={() => window.location.href = "/checkout?item=" + id + "&quantity=1"}
                                 className="text-sm font-inter font-semibold bg-transparent text-indigo-500 py-2 px-3 border border-indigo-500 rounded hover:cursor-pointer">
                                <div className="flex">
                                    <img src={buyNow} alt=""/>
                    ...
                    <div className="bg-white w-full border-[1px] p-4 border-purple-600 mt-4 mb-5 md:mt-0 md:ml-5 rounded-xl">
                        ...
                        <div className="ml-4">
                            <div className="flex">
                                {
                                    seller !== undefined
                                        ? <img onClick={() => window.location.href = "/user/" + seller?.username}
                                               src={seller?.profile_picture}
                                               className="hover:cursor-pointer h-11 w-11 border-purple-600 border-2 rounded-full"/>
                                ...
                                <div>
                                    {
                                        seller !== undefined
                                            ...
                                            :
                                            <div className="ml-4">
                                                <Skeleton variant="text" sx={{fontSize: '2rem'}} width={120} height={25}
                                                          className="mb-4"/>
                            ...
                            <Rating className="mt-1" precision={0.5} name="read-only" value={sellerRating / 2} readOnly
                                    size="small"/>
                        </div>
                    </div>
                ...
                <div className="flex justify-center">
                    <div className="p-5 w-full md:w-[86.5%] bg-white rounded-xl border-[1px] border-gray-300">
                        <div className="flex md:ml-14">
                            <svg className="w-6 h-6 fill-yellow-400 my-auto" clip-rule="evenodd" fill-rule="evenodd"
                            ...
                            <p className="font-inter font-bold text-gray-900 text-lg ml-4 mt-6 mb-6">Avaliações dos
                                usuários:</p>
                        </div>
                        {(reviews === undefined || reviews.length === 0)
                            ? <p className="text-gray-900 font-light text-md md:ml-16 md:text-left text-center">Nenhuma
                                avaliação até o momento.</p>
                            ...
                            border-indigo-400
                        `}>
                                        <Review reviewer={review.user.name} pfp={review.user.profile_picture}
                                                rating={review.rate} description={review.message}/>
                                    </div>
                                );
                            })}
                        <div className="md:mx-16 mx-2 mt-10">
                            <textarea value={review} onChange={(e: any) => setReview(e.target.value)}
                                      placeholder="Deixe uma avaliação para esse produto"
                                      className="w-full border-[1px] rounded-lg text-sm font-normal border-indigo-400 outline-none font-inter text-gray-700 p-3 resize-none"
                                      rows={2}/>
                            <div className="flex">
                                ...
                                <div className="ml-auto mr-0 mt-2 rounded-md bg-yellow-400 hover:cursor-pointer"
                                     onClick={postReview}>
